...
\author{\scriptsize{ Jeremy McEntire, Moriah Waterland, Kayla Trivieri, Z Braithwaite, Devon Stewart,  Tim Eller, Allys Ton, Justin Connor }}
\date{\normalsize{December 22, 2022}}
...
\begin{document}
	...
	\begin{tcolorbox}[colback=gray!0!white,colframe=gray!0!white, boxed title style={empty, size=minimal, bottom=1.5mm}]
		\abstract{
	appreciate is building a digital Proof of Ownership ecosystem. The appreciate platform generates Verifiable Credentials (VCs) to digitally link Items with their Owner's Decentralized Identifier (DiD). An Item's digital chain of custody begins with a Proof of Purchase (PoP) credential generated after the transaction receipt(s) ingestion. Owners manage their digital ownership of Items by providing additional data points on an item's availability, integrity, and authenticity. The Item's digital chain of custody informs the Proof of Ownership (POwn) calculation regarding an Owner's extant possession and ownership of an Item. Owners can request the POwn for an Item at any time to receive this probabilistic confidence score, known as the McEntire Score (M Score).
	
	The appreciate ecosystem design enables partners to provide services focused on their unique value add, minimizing additional technical overhead. The services may utilize platform primitives to gate access and functionality based on Owner provided PoP credentials or the M score for an Item. This secure-by-design architecture iterates on conventional Service Oriented Architecture (SOA) solutions by hosting logic inside a controlled environment rich with data and other constructs.
	
	As the ecosystem expands, we intend to capture PoP and M Score as on-chain Item Tokens (dynamic NFTs) and to enable commerce denominated in the appreciate fungible token. We look forward to contributing to the Owner Economy, and we are building not for consumers or users but for Owners.
