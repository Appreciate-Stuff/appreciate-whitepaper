...
\begin{document}
	...
	\begin{tcolorbox}[colback=gray!0!white,colframe=gray!0!white, boxed title style={empty, size=minimal, bottom=1.5mm}]
		\abstract{
			appreciate is building a digital Proof of Ownership ecosystem. The appreciate platform generates Verifiable Credentials (VCs) and Decentralized Identifiers (DiDs) to link Items with Owners digitally. An Item's digital chain of custody begins with a Proof of Purchase (PoP) credential generated after ingestion of the Item's receipt. Owners can manage the Item's provenance by providing additional data points on an item's availability, integrity, and authenticity. An Item's digital chain of custody informs the Proof of Ownership (POwn) calculation regarding an Owner's extant possession and ownership of the Item. When an Owner requests the POwn for an Item, a probabilistic confidence score, known as the McEntire Score (M Score), is generated.
			
			The appreciate ecosystem enables partners to provide services focused on their unique value add, minimizing additional technical overhead. The services may utilize platform primitives to gate access and functionality based on Owner provided PoP credentials or the M score for an Item. This secure-by-design architecture iterates on conventional Service Oriented Architecture (SOA) solutions by hosting logic inside a controlled environment rich with data and other constructs.
			
			As the ecosystem expands, we intend to capture PoP and M Score as on-chain Item Tokens (dynamic NFTs) and to enable commerce denominated in the appreciate fungible token. We look forward to contributing to the Owner Economy and are building for Owners rather than consumers or users.
